...
`go-ruby-vcr/vcr` reimplements Ruby's `vcr` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-vcr/vcr` directly, with no Ruby runtime;
...
Because `vcr` is pure Go with cgo disabled, it also compiles to **WebAssembly**
